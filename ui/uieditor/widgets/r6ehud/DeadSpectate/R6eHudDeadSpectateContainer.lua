...
require( "ui.uieditor.widgets.r6ehud.DeadSpectate.R6eHudDeadSpectateInternal" )

CoD.R6eHudDeadSpectateContainer = InheritFrom( LUI.UIElement )
CoD.R6eHudDeadSpectateContainer.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.R6eHudDeadSpectateContainer )
	self.id = "R6eHudDeadSpectateContainer"
	self.soundSet = "default"
	self:setLeftRight( true, false, 0, 300 )
	self:setTopBottom( true, false, 0, 60 )
	self.anyChildUsesUpdateState = true
	
	local internal = CoD.R6eHudDeadSpectateInternal.new( menu, controller )
